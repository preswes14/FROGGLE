...
// Vertical list — each gambit is a full-width row
options.forEach(opt => {
...
html += `<div class="choice" onclick="selectD20Action(${heroIdx}, ${adjustedDC}, '${opt.name}')" style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.4rem;padding:0.5rem 0.75rem;cursor:pointer">
<strong style="min-width:4.5rem;font-size:0.95rem">${dcText}</strong>
<strong style="min-width:5.5rem;font-size:1.05rem;color:#3b82f6">${opt.name}</strong>
...
html += `<div style="text-align:center"><img src="${heroImage}" alt="${h.n}" class="hero-portrait" style="object-fit:${crop.fit};object-position:${crop.pos};filter:sepia(30%) brightness(0.8);${heroFlipStyle(heroImage)}"></div>`;
}
...
html += `<img src="${heroImage}" alt="${h.n}" class="hero-portrait" style="object-fit:${crop.fit};object-position:${crop.pos};${heroFlipStyle(heroImage)}">`;
}
...
html += `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.25rem;gap:0.15rem">`;
html += `<div style="font-size:1rem;font-weight:bold;min-width:1.2em;text-align:center;flex-shrink:0">${e.p}</div>`;
