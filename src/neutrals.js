...
return `<div style="background:white;border:3px solid #000;border-radius:12px;padding:1.25rem;flex:1;min-width:0;color:#1a1a1a;display:flex;flex-direction:column;gap:0.85rem">
<div style="display:flex;justify-content:space-between;align-items:center;gap:0.5rem">
<h3 style="font-size:1.35rem;margin:0;color:#1a1a1a">Slot ${num}</h3>
${meta.lastSaved ? `<span style="font-size:0.85rem;color:#6b7280">${formatSaveDate(meta.lastSaved)}</span>` : ''}
</div>
${meta.exists ? `
<div style="font-size:1rem;color:#374151;flex:1;display:flex;flex-direction:column;gap:0.35rem">
...
<button class="btn" onclick="continueSlot(${num})" style="width:100%;padding:0.85rem 1rem;margin:0;background:#22c55e;border:3px solid #16a34a;font-weight:bold;font-size:1.1rem;overflow:visible">${meta.hasActiveRun ? 'Continue' : 'New Run'}</button>
<button class="btn secondary" onclick="confirmDeleteSlot(${num})" style="width:100%;padding:0.6rem 1rem;margin:0;font-size:0.95rem;min-height:auto;overflow:visible">Delete Save</button>
</div>
` : `
<p style="color:#6b7280;font-size:1.05rem;flex:1;margin:0;display:flex;align-items:center;justify-content:center;text-align:center">Empty Slot</p>
<button class="btn" onclick="createNewSlot(${num})" style="width:100%;padding:0.85rem 1rem;margin:0;background:#3b82f6;border:3px solid #f97316;font-weight:bold;font-size:1.1rem;overflow:visible">New Game</button>
`}
</div>`;
}
...
v.innerHTML = `
<div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#1a1a1a;padding:1.5rem;overflow:hidden;box-sizing:border-box">
<div style="background:#22c55e;border:4px solid #000;border-radius:16px;padding:1.75rem;max-width:1200px;width:100%;box-shadow:0 8px 16px rgba(0,0,0,0.5)">
<h2 style="text-align:center;margin:0 0 1.5rem 0;font-size:1.75rem;color:#000">Select Save Slot</h2>
...
<button class="btn secondary" onclick="mainTitlePage()" style="width:100%;padding:0.75rem;margin:1.25rem 0 0 0;font-size:1rem">← Back</button>
</div>
</div>`;
}
...
<h2 style="font-size:1.5rem;text-align:center">Alright champ!</h2>
<p style="text-align:center">
No time to waste, eh? Well then get going! Save Tapo!
</p>
<div style="background:rgba(59,130,246,0.1);border:2px solid #3b82f6;border-radius:12px;padding:0.75rem;margin-bottom:0.5rem">
<p style="font-size:0.95rem;text-align:center;margin-bottom:0.5rem">
...
overlay.innerHTML = `
<div class="tutorial-modal" style="padding:1rem 1.5rem;background:rgba(31,41,55,0.95);border-width:3px;">
<p>Here come three <strong>flies</strong> now - you're up!</p>
<button onclick="dismissTaposBirthdayOverlay()" class="tutorial-btn">Let's catch flies!</button>
</div>`;
document.body.appendChild(overlay);
}
...
<div class="tutorial-modal">
<div style="text-align:center">
<div style="animation:tapoSignatureRescue 3s ease-in-out infinite;display:inline-block;margin-bottom:0.5rem">
<img src="assets/tapo_normal.png" alt="Tapo" style="width:100px;height:auto;transform:scaleX(-1)">
</div>
<h2 style="color:#22c55e">Tapo to the Rescue!</h2>
<p>
...
<button onclick="continueTapoRescue()" class="tutorial-btn" style="padding:0.75rem 2.5rem;font-size:1.2rem">
*ribbit*
</button>
</div>
</div>
...
if(available.length === 0) {
outcomes.push('All your sigils are already at maximum power.');
outcomes.push('"Impressive. Well, take this at least \u2014 consider it an investment in your longevity."');
buttons = `<button class="btn safe" onclick="offerDeathsConsolation()">Accept the gift</button><button class="btn secondary" onclick="declineDeathsConsolation()">Decline</button>`;
} else if(S.gold < cost) {
outcomes.push('<span style="color:#dc2626">"Darn. Not enough gold. Maybe next time, I guess?"</span>');
outcomes.push('"Tell you what \u2014 take this. Consider it an investment in your longevity. I\'d hate for this run to end too soon."');
buttons = `<button class="btn safe" onclick="offerDeathsConsolation()">Accept the gift</button><button class="btn secondary" onclick="declineDeathsConsolation()">Decline</button>`;
} else {
...
// Player declined the sigil upgrade — offer consolation gift instead
const v = document.getElementById('gameView');
v.innerHTML = buildNeutralHTML({
bgImage: 'assets/neutrals/shopkeeper2.png',
outcomes: ['"Really, you\'re not gonna take this deal? Okay... Well, take this at least."'],
buttons: `<button class="btn safe" onclick="offerDeathsConsolation()">Accept the gift</button><button class="btn secondary" onclick="declineDeathsConsolation()">Decline</button>`
});
}

function declineDeathsConsolation() {
removeNeutralFromDeck('shopkeeper');
const v = document.getElementById('gameView');
v.innerHTML = buildNeutralHTML({
bgImage: 'assets/neutrals/shopkeeper2.png',
...
outcome = 'You climb carefully and manage to grab a few coins, but get a booboo climbing out of the well. (+3 Gold)';
hpLoss = 1;
goldGain = 3;
} else if(best >= 11 && best <= 19) {
outcome = 'Your climbing skills are impressive! You retrieve a small cache of coins from the bottom. (+5 Gold)';
goldGain = 5;
} else if(best === 20) {
...
description: 'A closed, unadorned wooden chest lies in the center of this next room. As you draw near, you notice the chest pockmarked with dried blood.',
buttons: `
...
if(chestOpened) {
// Success! Roll again to determine gold contents
if(footer) {
footer.innerHTML = `<button class="btn" onclick="openChestGoldRoll()">See what's inside</button>`;
}
} else {
...
function openChestGoldRoll() {
const v = document.getElementById('gameView');
v.innerHTML = buildNeutralHTML({
bgImage: 'assets/neutrals/treasurechest1.png',
title: 'Chest Contents',
description: 'Gold glints inside the chest! How much can you grab?',
diceTray: renderDiceTray({ rollCallback: 'openChestGoldResult()' })
});
}

function openChestGoldResult() {
animateDiceRoll(function(goldRolls, goldBest) {
const goldGain = goldBest;

replaceStage1WithStage2('treasurechest');

const narrative = document.querySelector('.neutral-narrative');
if(narrative) {
narrative.innerHTML = `<div class="neutral-title">Chest Contents</div><div class="neutral-outcome">You find <strong>${goldGain} gold</strong> inside the chest!</div>`;
}
...
description: 'You can hear the clink of metal as soon as you open the chamber. A small, elegant silver chest sits in the middle of the room, but from the way it periodically opens and closes, you easily deduce it is a mimic.',
buttons: `<button class="btn risky" onclick="openMimicChest()">Grab as much gold as you can!</button>`
});
}

function openMimicChest() {
const v = document.getElementById('gameView');
v.innerHTML = buildNeutralHTML({
bgImage: 'assets/neutrals/treasurechest2.png',
title: 'The Silver Chest',
description: 'Grab as much gold as you can from its mouth before it spots you!',
diceTray: renderDiceTray({ rollCallback: 'mimicChestRoll()' })
});
}

function mimicChestRoll() {
animateDiceRoll(function(rolls, best) {
const goldGain = rolls.reduce((sum, r) => sum + r, 0);
const rollBreakdown = rolls.length > 1 ? `(${rolls.join(' + ')}) = ` : '';
...
const narrative = document.querySelector('.neutral-narrative');
if(narrative) {
narrative.innerHTML = `<div class="neutral-title">The Silver Chest</div><div class="neutral-outcome">Not an especially fast mimic, apparently \u2014 your team made it out with a whopping <strong>${rollBreakdown}${goldGain} gold</strong>!</div>`;
}
