...
const skipButton = slides.skippable ? `<button class="btn" onclick="skipTutorialFromSlide()" style="padding:0.5rem 1rem;background:rgba(100,100,100,0.8);border:2px solid #666;font-size:0.85rem">Skip</button>` : '';
...
<div style="position:absolute;bottom:0;left:0;right:0;z-index:10;background:linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.85) 60%, rgba(0,0,0,0.4) 85%, transparent 100%);padding:1rem 1rem 0.75rem 1rem">
...
<button class="btn" onclick="continueNarrative()" style="padding:0.5rem 1.5rem;font-size:0.95rem;background:#22c55e;border:2px solid #15803d">${slide.buttonText || 'Continue'}</button>
${skipButton}
</div>
...
v.innerHTML = `
<div style="width:100%;height:calc(100vh - 44px);display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden;padding:0.5rem">
<div style="max-width:550px;text-align:center">
${slide.html || `<div style="font-size:1rem;line-height:1.6;margin-bottom:1rem">${slide.text}</div>`}
<div style="display:flex;gap:0.75rem;justify-content:center;flex-wrap:wrap;margin-top:0.75rem">
<button class="btn" onclick="continueNarrative()" style="padding:0.6rem 1.5rem;font-size:1rem">${slide.buttonText || 'Continue'}</button>
${skipButton}
</div>
<div style="margin-top:0.5rem;font-size:0.8rem;opacity:0.6">Ⓐ to continue${slides.skippable ? ' • Ⓑ to skip' : ''}</div>
</div>
</div>`;
}
...
html: `
<h2 style="font-size:1.5rem;margin-bottom:0.75rem">Today is <strong style="color:#22c55e">Tapo's First Birthday!</strong> 🎂</h2>
<div style="animation:tapoSignature 4.8s ease-in-out infinite;display:inline-block;margin:0.75rem 0">
<img src="assets/tapo-nobg.png" style="width:140px;height:auto">
</div>
<p style="font-size:1rem;line-height:1.5;margin-top:0.5rem">The whole town is celebrating the little tadpole's special day!</p>
<style>
@keyframes tapoSignature {
  0% { transform: translateY(0) scaleX(1); }
  10% { transform: translateY(-20px) scaleX(1); }
  20% { transform: translateY(0) scaleX(1); }
  30% { transform: translateY(-28px) scaleX(1); }
  40% { transform: translateY(0) scaleX(1); }
  45% { transform: translateY(-8px) scaleX(0); }
  50% { transform: translateY(0) scaleX(-1); }
  60% { transform: translateY(-20px) scaleX(-1); }
  70% { transform: translateY(0) scaleX(-1); }
  80% { transform: translateY(-28px) scaleX(-1); }
  90% { transform: translateY(0) scaleX(-1); }
  95% { transform: translateY(-8px) scaleX(0); }
...
{
html: `
<h2 style="font-size:1.4rem;margin-bottom:0.5rem;color:#2c63c7">A Special Gift</h2>
<div style="display:flex;justify-content:center;align-items:center;gap:1.25rem;margin:0.75rem 0">
<div>
<img src="assets/characters/magefull.png" style="width:100px;height:auto;border-radius:6px;border:2px solid #22c55e;box-shadow:0 2px 6px rgba(0,0,0,0.2)">
<div style="margin-top:0.25rem;font-weight:bold;font-size:0.9rem">Mage</div>
</div>
<div style="font-size:2rem">🎁</div>
<div style="animation:tapoSignature 4.8s ease-in-out infinite">
<img src="assets/tapo-nobg.png" style="width:80px;height:auto">
<div style="margin-top:0.25rem;font-weight:bold;font-size:0.9rem">Tapo</div>
</div>
</div>
<p style="font-size:1rem;line-height:1.5;margin-top:0.5rem">
...
<h2 style="font-size:1.5rem;margin-bottom:0.75rem;color:#22c55e">Success!</h2>
<p style="font-size:1rem;line-height:1.6;margin:0.5rem 0">
...
<button onclick="transitionToPortalInvasion()" style="padding:0.6rem 1.5rem;font-size:1rem;font-weight:bold;background:#22c55e;color:#fff;border:2px solid #15803d;border-radius:8px;cursor:pointer;margin-top:0.75rem">Continue</button>
</div>
</div>`;
}
...
{
html: `
<h2 style="font-size:1.4rem;margin-bottom:0.5rem;color:#dc2626;animation:shake 0.5s ease-in-out infinite">DANGER!</h2>
<div style="margin:0.75rem 0;position:relative">
<div style="width:120px;height:120px;margin:0 auto;position:relative;border-radius:50%;background:radial-gradient(circle, #dc2626, #7c2d12);animation:narrativePortalPulse 1s ease-in-out infinite;box-shadow:0 0 30px #dc2626"></div>
<div style="position:absolute;top:50%;left:50%;transform:translate(-50%, -50%);font-size:2.5rem;animation:spin 2s linear infinite">🌀</div>
</div>
<p style="font-size:0.95rem;line-height:1.5;margin:0.5rem 0">
As Mage and Tapo enter Ribbleton, something seems off..<br>
Whoa! <strong>A dark portal</strong> is open in the center of square!
</p>
<div style="display:flex;justify-content:center;gap:1.5rem;margin:0.75rem 0;font-size:2rem">
...
<div style="font-size:2.5rem;font-weight:bold;margin-bottom:0.75rem">FROGGLE</div>
<div style="font-size:1.2rem;font-style:italic">A Froggy Roguelike</div>
</div>
</div>`;
