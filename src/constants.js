...
// Each pip = level tier color (L1=silver, L2=teal, L3=purple, L4=orange, L5=prismatic)
const PIP_COLORS = ['#c0c0c0', '#2dd4bf', '#a855f7', '#f59e0b', '#ff0080'];
const PIP_POS = [
  [],                                           // 0
  [[0, 0]],                                     // 1: single dot
...
html += '</div></div>';
// Fixed-position confirm bar at the bottom of the viewport so it's always visible even when
// the combat-grid fills the screen. Previously the button was appended after the grid in
...
html += `<div class="encampment-confirm-bar" style="display:flex;gap:0.75rem;align-items:center;background:rgba(0,0,0,0.8);padding:0.6rem 1rem;border-radius:12px;border:2px solid ${canConfirm ? '#dc2626' : 'rgba(255,255,255,0.2)'};box-shadow:0 4px 16px rgba(0,0,0,0.5)">
<span style="color:#fff;font-weight:bold;font-size:0.95rem">${selected}/${kills} selected</span>
<button class="btn danger" ${canConfirm ? '' : 'disabled'} onclick="confirmEncampmentKills()" style="padding:0.5rem 1.25rem;margin:0;min-height:auto;font-size:0.95rem;${canConfirm ? '' : 'opacity:0.5;cursor:not-allowed'}">Confirm Removals</button>
</div>`;
return html;
}
