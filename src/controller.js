...
const GamepadController = {
  ...
  init() {
    ...
    // Try to initialize Steam Input first (preferred for Steam Deck)
    this.initSteamInput();
  ...
  // Initialize Steam Input (native controller support for Steam/Steam Deck)
  initSteamInput() {
    // Check if Steam Input bridge is available (only in Electron with Steam)
    if (typeof steamInputBridge === 'undefined') {
      debugLog('[STEAM INPUT] Bridge not available (not running in Steam)');
      return;
    }

    if (!steamInputBridge.initialized) {
      debugLog('[STEAM INPUT] Not initialized');
      return;
    }

    debugLog('[STEAM INPUT] Initializing...');
    this.steamInputActive = true;
    this.activateControllerMode();
    toast('Steam Input controller connected!', 2000);

    // Register for input state updates from main process
    steamInputBridge.onInput((state) => {
      this.processSteamInput(state);
    });

    debugLog('[STEAM INPUT] Listening for input');
  },

  // Process Steam Input state and trigger actions
  processSteamInput(state) {
    if (!this.steamInputActive) return;

    const prev = this.steamInputPrevState;
    const cooldown = 100; // ms between repeated actions
    const now = Date.now();

    // Helper to detect button press (transition from not pressed to pressed)
    const justPressed = (action) => state[action] && !prev[action];

    // Helper to check cooldown
    const canAct = () => {
      if (now - this.lastInputTime < cooldown) return false;
      this.lastInputTime = now;
      return true;
    };

    // Process digital actions (buttons)
    if (justPressed('menu') && canAct()) {
      this.openMenu();
    } else if (justPressed('confirm') && canAct()) {
      if (!this.tryDirectAction()) {
        this.confirmSelection();
      }
    } else if (justPressed('cancel') && canAct()) {
      this.goBack();
    } else if (justPressed('autoTarget') && canAct()) {
      this.autoTarget();
    } else if (justPressed('tooltip') && canAct()) {
      this.toggleTooltip();
    } else if (justPressed('prevUnit') && canAct()) {
    ...
    } else if (justPressed('nextUnit') && canAct()) {
      const context = this.getNavigationContext();
      if (context === 'combat' || context === 'targeting') {
        this.cycleUnit('next');
      } else {
    ...
    } else if (justPressed('prevSigil') && canAct()) {
      const context = this.getNavigationContext();
      if (context === 'combat' || context === 'targeting') {
        this.cycleSigil('prev');
      }
    } else if (justPressed('nextSigil') && canAct()) {
      const context = this.getNavigationContext();
      if (context === 'combat' || context === 'targeting') {
        this.cycleSigil('next');
      }
    } else if (justPressed('switchSides') && canAct()) {
      this.switchSides();
    } else if (justPressed('dpadUp') && canAct()) {
      this.onDirection('up');
    } else if (justPressed('dpadDown') && canAct()) {
      this.onDirection('down');
    } else if (justPressed('dpadLeft') && canAct()) {
      this.onDirection('left');
    } else if (justPressed('dpadRight') && canAct()) {
      this.onDirection('right');
    }

    // Process analog stick movement
    if (state.move && canAct()) {
      const deadzone = 0.5;
      const x = state.move.x || 0;
      const y = state.move.y || 0;
    ...
    this.steamInputPrevState = { ...state };
  },
